      ...
      `(module ,name racket/base
         ...
         ,@(for/list ([f (in-vector (store-functions s))]
             ...
             `(define (,(func-name i) ,@arg-regs)
                (comment ,(or (func-debug-name m i) "anonymous"))
                ,@(for/list ([id (in-list local-regs)])
                    `(define ,id #f))
                ,@(try-eliminate-return
                   (resugar
                    (try-eliminate-label
                     `(let/cc $return
...
(define (try-eliminate-return e)
  (match e
    [`(let/cc $return
        ,es ...
        ($return ,ret-vals ...)
        (trap "unreachable") ...)
     #:when (not (label-used? '$return es))
     `(,@es (values ,@ret-vals))]
